...
def get_filing_text_and_isolate_risks(url: str, user_agent: str, filing_type: str) -> str:
    ...
    if filing_type.strip().upper() == "8-K":
        soup = BeautifulSoup(html_text, "html.parser")
        for element in soup(["script", "style", "noscript"]):
            element.decompose()
        text = soup.get_text(separator=" ")
...
def _build_audit_prompts(risk_factors_text: str, source_context: str) -> tuple[str, str]:
    system_prompt = (
    ...
    prompt = (
        ...
        "a company's latest 10-K filing. Use the exact source context below for every citation:\n"
        f"{source_context}\n\n"
...
def run_forensic_audit_analysis(
    *,
    ...
    system_prompt, prompt = _build_audit_prompts(risk_factors_text, source_context)
...
def main() -> None:
    ...
    with st.sidebar.container():
        ...
        filing_type = st.sidebar.selectbox(
            ...
            ["10-K", "10-Q", "8-K"],
            index=0,
            help="Choose the latest SEC filing form to inspect. 10-K and 10-Q get section extraction; 8-K falls back to the filing text.",
        )
    ...
    if analyze_btn and ready_to_analyze:
        ...
        try:
            ...
            source_context = (
                f"Source: {filing_info['form_type']} filing, SEC filing {filing_info['url']} | "
                f"Company: {filing_info['company_name']} | Filing date: {filing_info['filing_date']} | "
                f"Report date: {filing_info['report_date']}"
            )
