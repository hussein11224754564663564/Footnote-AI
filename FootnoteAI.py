...
def main() -> None:
    st.markdown("<div class='main-header'>ðŸ’¼ Financial Forensic Audit Assistant</div>", unsafe_allow_html=True)
    st.markdown(
        "<div class='sub-header'>Programmatic Accounting & AI Risk Screening for SEC 10-K Disclosures</div>",
        unsafe_allow_html=True,
    )

    st.sidebar.markdown("<div class='sidebar-title'>ðŸ”¬ Audit Configurations</div>", unsafe_allow_html=True)
    ...
    if secret_api_key:
        st.sidebar.caption(f"{provider_name} API key loaded from Streamlit Secrets.")
    ...
    provider_model = st.sidebar.text_input(
        ...
        value=default_model_map[provider_name],
        help="You can change this to any model supported by the provider.",
    ).strip()
    ...
    st.sidebar.caption(
        "Tip: use Streamlit Secrets to keep API keys out of the sidebar if you prefer."
    )
    ...
    analyze_btn = st.sidebar.button("ðŸš€ Fetch & Audit Filing", disabled=not ready_to_analyze, use_container_width=True)
    ...
    if analyze_btn and ready_to_analyze:
        st.session_state.audit_data = None
        status_bar = st.status("ðŸ•µï¸ Initiating forensic extraction...", expanded=True)

        try:
            status_bar.update(label="ðŸ”‘ Resolving Stock Ticker to SEC CIK...", state="running")
            normalized_ticker = normalize_ticker(ticker)
            cik = get_cik_by_ticker(normalized_ticker, user_agent)

            status_bar.update(label="ðŸ“‚ Crawling SEC Submissions for latest 10-K...", state="running")
            filing_info = get_latest_10k_info(cik, user_agent)

            status_bar.update(label="ðŸ“„ Downloading & isolating Item 1A. Risk Factors...", state="running")
            risk_text = get_filing_text_and_isolate_risks(filing_info["url"], user_agent)

            status_bar.update(label="ðŸ“ˆ Fetching XBRL company facts & financial metrics...", state="running")
            fin_df, rev_tag, ar_tag = get_financial_data(cik, user_agent)

            status_bar.update(label="ðŸ¤– Feeding Risk Factors to Senior Forensic Auditor Agent...", state="running")
            provider = ProviderSelection(
                name=provider_name,
            ...
            ai_analysis, ai_warning = run_forensic_audit_analysis(provider=provider, risk_factors_text=risk_text)
            if ai_warning:
                st.warning(ai_warning)

            status_bar.update(label="âœ… Forensic screening complete!", state="complete")
    ...
    tab1, tab2, tab3 = st.tabs(["ðŸš¨ Audit Red Flags & Risk Factors", "ðŸ“ˆ Quantitative Anomalies", "â„¹ï¸ About the Project"])
    ...
    with tab1:
        if st.session_state.audit_data is None:
            ...
            st.markdown(f"[ðŸ”— View Original Filing on SEC Archives]({data['url']})")
            st.divider()

            st.markdown("### ðŸ•µï¸ Senior Forensic Auditor Assessment")
            st.markdown(data["ai_analysis"])

            st.divider()
            with st.expander("ðŸ“„ View Isolated Raw Text of Item 1A. Risk Factors"):
    ...
    with tab2:
        if st.session_state.audit_data is None:
            ...
            st.markdown(f"## ðŸ“ˆ Quantitative Anomaly Analysis for {data['ticker']}")
            st.markdown(
                ...
                st.markdown(
                    ...
                    <div class="metric-card">
                        ...
                        <div style="font-size: 13px; color: {'#10b981' if latest_rev_growth >= 0 else '#ef4444'}; margin-top: 5px; font-weight: bold;">
                            {'â–²' if latest_rev_growth >= 0 else 'â–¼'} {latest_rev_growth:+.2f}% YoY
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                st.markdown(
                    ...
                    <div class="metric-card">
                        ...
                        <div style="font-size: 13px; color: {'#ef4444' if latest_ar_growth > latest_rev_growth else '#10b981'}; margin-top: 5px; font-weight: bold;">
                            {'â–²' if latest_ar_growth >= 0 else 'â–¼'} {latest_ar_growth:+.2f}% YoY
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                st.markdown(
                    ...
                    <div class="metric-card">
                        ...
                        <div style="font-size: 13px; color: {'#ef4444' if dso_yoy > 5 else '#10b981'}; margin-top: 5px; font-weight: bold;">
                            {'â–²' if dso_yoy >= 0 else 'â–¼'} {dso_yoy:+.2f}% YoY
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
            ...
            st.markdown("### ðŸ”” Automated Forensic Anomaly Flags")
            ...
            if latest_row["dso_change_pct"] > DSO_SPIKE_THRESHOLD_PCT:
                ...
                st.markdown(
                    ...
                    <div class="alert-card critical-alert">
                        <div class="alert-header">ðŸš¨ CRITICAL FLAG: Significant Spike in Days Sales Outstanding (DSO)</div>
            ...
            if latest_div > AR_REV_DIVERGENCE_THRESHOLD_PCT:
                ...
                st.markdown(
                    ...
                    <div class="alert-card warning-alert">
                        <div class="alert-header">âš ï¸ WARNING FLAG: Accounts Receivable Outgrowing Revenue YoY</div>
                        <div class="alert-desc">
            ...
            if not anomaly_detected:
                st.markdown(
                    ...
                    <div class="alert-card healthy-alert">
                        <div class="alert-header">âœ… NO CONSPICUOUS QUANTITATIVE ANOMALIES FOUND</div>
                        <div class="alert-desc">
            ...
            st.markdown("### ðŸ“‹ SEC Historical Data Extract Table")
            display_df = df.copy()
    ...
    with tab3:
        st.markdown(
            ...
            ### ðŸŽ“ Forensic Audit Rules Explained
